...
// ETAPE PRIMAIRE : Connexion à la base de données
...
// Je vais maintenant créer une condition pour la modification des clients de la table annuaire
if (isset($_GET['action']) && $_GET['action'] == 'update') {
  // Je récupère l'id_annuaire de l'url
  $id = $_GET['id'];
  // Je prépare la requête de modification en me connectant à la BDD
  $reqUpdate = $connect_bdd->prepare("UPDATE annuaire SET  WHERE id_annuaire = :id");
  // Je lie l'id_annuaire à la requête de modification
  $reqUpdate->bindValue(':id', $id, PDO::PARAM_INT);
  // J'exécute la requête de modification
  $reqUpdate->execute();
}
